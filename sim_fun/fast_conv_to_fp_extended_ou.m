function [xout,z,xrec]=fast_conv_to_fp_extended_ou(net,f_ol,opt)
...
dW=sqrt(opt.dt)*opt.sigext*randn(net.N,size(xout,2),nsteps);
...
for t=1:nsteps
    ...
    if opt.ol
        xout=xout+opt.dt*(...
            ...
            +net.wfb*f_ol...
            +net.win*opt.u_in(t)...
            +opt.u*ones(1,size(f_ol,2)));
    else
        xout=xout+opt.dt*(...
            -xout...
            +(net.W+net.wfb*net.wout')*net.phi(xout)...
            +net.win*opt.u_in(:,t)...
            +opt.u*ones(1,size(xout,2)))+dW(:,:,t);
    end
