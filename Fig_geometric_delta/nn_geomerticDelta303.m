...
for psi_psi = 1:length(psi_vec)
    ...
    u_in_transient{psi_psi}=create_step_input(u{psi_psi}(:,1),ceil(tmax/dt),floor(tswitch/dt));
...
[xtran,ztran{psi_psi},xrectran]=fast_conv_to_fp_extended(net,[],...
    struct('xinit',x(:,1),...
    'ol',0,...
    'tmax',tmax,...
    'dt',dt,...
    'u_in',u_in_transient{psi_psi},...
    'save_neurons',[1:50:100]));
...
%         delta_z_geom_perp{psi_psi}=f_geom{psi_psi}-sim.f_ol;
    ...
    plot(sim.psi,delta_z_geom_par{psi_psi},'*')
...
figure;
for psi_psi = 1:length(psi_vec)
    dpsidt{psi_psi}=diff(phase(ztran{psi_psi}));
    plot(phase(ztran{psi_psi}(2:end)),dpsidt{psi_psi},'.');
    hold on;
