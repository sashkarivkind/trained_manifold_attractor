...
%         Amp = A_vec(jj);
        J_0 = J_vec(jj);
        [G,debuu{jj,mm},sfpr{jj,mm}]=nn_MFT_Fourier(phi,J_ext,J_0,Amp,K,psi_vec,phip,omega_vec,0,24);
        Grr_rec{jj,mm}=G.rr;
        Gpp_rec{jj,mm}=G.pp;
...
for jj=1:length(J_vec)
mm=1;
MM=(debuu{jj,mm}.pp_p1.v1+1-debuu{jj,mm}.pp_p1.v2)-eye(length(debuu{jj,mm}.pp_p1.v2));
[v,e]=eig(MM,'vector')
plot(v(:,1));hold on;
end
%%
mm=1;
jj=1;
MMMM=1.2*debuu{jj,mm}.betapp{1}+debuu{jj,mm}.betapp{2};
[v,e]=eig(MMMM,'vector');
e
plot(v(:,1));hold on;