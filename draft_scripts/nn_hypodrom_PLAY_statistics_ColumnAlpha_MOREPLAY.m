...
cp=(rp'*rp);
I28_debu{end+1}=eta'*diag(cp(1,:))*eta*diag(lambda);

beta0_rescaled_rec{end+1}=inv(lhs)*B0*((qn(dd,:).*Gnprefac'));
M_rescaled_rec{end+1}=MM;
y_rescaled_rec{end+1}=y;
...
beta0_rec{end+1}=B0*((qn(dd,:).*Gnprefac'));
M_rec{end+1}=MM_m;
beta_rec{end+1}=B1;
dd_rec{end+1}=dd;
%%
...
zz=[];
for qq=1:5
%     zz(qq,:,:)=log10(abs(beta_rescaled_rec{qq}));
