...
pts = 1:20:80;
wout1 = (sim.r(:,pts)/...
    (sim.r(:,pts)'*sim.r(:,pts)))*sim.f_ol(:,pts)'; %obtain least mean square solution
%%
[u,s,v]=svd(sim.r(:,pts)');
invs=s;
invs(s~=0)=1./invs(s~=0);
wout1a = sim.f_ol(:,pts)*u*invs*v';
wout1a=wout1a';
%%
hhh=80;
th=1e-10;
[u,s,v]=svd(sim.r(:,1:hhh)');
invs=s;
invs(s>th)=1./invs(s>th);
fol_detlas=sim.f_ol(:,1:hhh)*diag(ismember(1:hhh,pts));
wout2a = fol_detlas*u*invs*v';
wout2a=wout2a';

zz=wout2a'*sim.r;
figure; plot(zz(1,:),zz(2,:));