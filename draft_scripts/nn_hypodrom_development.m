...
eigJW={};
% % h_vec=[-0.9:0.1:0.9];
h_vec=[-0.9:0.2:-0.3 -0.2:0.04:0.2 0.3:0.2:0.9];
            ...
            for hh=1:length(h_vec)
                ...
                for kk=1:length(K_vec)
                    ...
                    for dd=1:2
                        ...
                        wout_pc=net.wout'*rpc/diag(sqrt(lambdaK))/sqrt(Meff);
...
%                         dd_rec{gg,hh,mm,jj,dd}=dd;
                        qn_rec{gg,hh,mm,jj,dd,kk}=qn;
                        wout_pc_rec{gg,hh,mm,jj,dd,kk}=wout_pc;
                    end
                end
...
figure;
for hh=1:length(h_vec)
%     semilogy(lambdaK'.^(1/2).*abs(qn_rec{1,hh,1,1,2,end}(2,:)));
%     hold on;
%         semilogy(abs(qn_rec{1,hh,1,1,2,end}(2,:)));
%             semilogy(sort(abs(wout_pc_rec{1,hh,1,1,2,end}(2,:)),'descend'));
            semilogy((abs(wout_pc_rec{1,hh,1,1,2,end}(2,:))));
    hold on;

end
legend(num2str(h_vec'));
%% Load
IT=1;
for jj=1:IT
    jj
    ...
    for jj=1:1
        for mm=2:length(M_vec)
            for kk=[kmax]%:length(K_vec)]
                if dd==1
                    plot(h_vec,squeeze(maxEvR(gg,:,mm,jj,kk)),'--','Color',colors(mm*10,:)); hold on
                else
                    if logFlag
                    semilogy(h_vec,squeeze(abs(maxEvPsi(gg,:,mm,jj,kk))),'--','Color',colors(mm*10,:)); hold on
                    else
                     plot(h_vec,squeeze((maxEvPsi(gg,:,mm,jj,kk))),'--','Color',colors(mm*10,:)); hold on
                    end
                end
