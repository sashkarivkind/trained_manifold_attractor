...
for aa=1:length(A_vec)
    ...
    for gg=1:length(g_vec)
        ...
        for sese=1:length(seed_vec)
            try
            loaded=load(['results/HRresultsN',num2str(N),'iter',num2str(cnt)],'result','N','g','A','seed');
            cnt=cnt+1;
...
dmat_approx=cellfun(@(xx) xx.d_approx, mft_result_table);
Amat=cellfun(@(xx) xx.A, mft_result_table);
rmsmat=dmat./Amat/sqrt(N);
rmsmat_approx=dmat_approx./Amat/sqrt(N);
for aa=3%1:length(A_vec)
cc1=abs(A_rec-A_vec(aa))<1e-8;
plot(g_rec(cc1),rmse_rec(cc1),'x');
hold on;
rmse_erb=my_prep_errb(g_rec(cc1),rmse_rec(cc1));
errorbar(rmse_erb.xx,rmse_erb.yy_m,rmse_erb.yy_std./sqrt(rmse_erb.yy_n-1),'linewidth',3);

% set(gca,'ColorOrderIndex',get(gca,'ColorOrderIndex')-1);
% plot(g_vec(1:end-10),rmsmat(aa,1:end-10),'-');
plot(g_vec(1:end-10),rmsmat_approx(aa,1:end-10),'-','linewidth',3);
end
legend('data', 'mean \pm stderr', 'theory');
xlabel('g')
ylabel('rms \Delta')
title(['A=', num2str(A_vec(aa))]);
%%
figure;
subplot(2,2,1)
dmat=cellfun(@(xx) xx.d, mft_result_table);
