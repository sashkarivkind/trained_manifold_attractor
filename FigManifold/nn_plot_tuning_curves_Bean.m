function [result]=nn_plot_tuning_curves_Bean(a,h,A,g,N,M,seed,UniformFlag,PlotFlag,PlotEigFlag)
% 22/01/21
...
hp.M=M;
if UniformFlag
    rBean=a*(sin(sim.psi-rot).^3+cos(sim.psi-rot).^3);
    tmp_f_ol=hp.A*rBean.*[cos(sim.psi-rot);sin(sim.psi-rot)];
    sim.f_ol=curvspace(tmp_f_ol',hp.sim_resolution);
    sim.f_ol=sim.f_ol';
else
    rBean=a*(sin(sim.psi-rot).^3+cos(sim.psi-rot).^3);
    sim.f_ol=hp.A*rBean.*[cos(sim.psi-rot);sin(sim.psi-rot)];
end
