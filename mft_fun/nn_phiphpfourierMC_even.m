function [BETA0,BETA1]=phiphpfourierMC_even(phi,phip,c,A,nmc)
...
for ii=1:length(reCo)
    BETA0(ii)=2*mean(...
        cos((2*ii-1)*tp(:,2)).*...
        phi(A*cos(sum(tp,2))+x1_approx)...%phi integrated over psi
        .*phip(A*cos(tp(:,1))+sum(ai,2))...%at origin we have a sum of all the cosine coeffitients
        .*cos(tp(:,1))...
        );
    for jj=1:length(reCo)
        BETA1(ii,jj)=2*mean(...
            cos((2*ii-1)*tp(:,2)).*... %todo - hard coded only odd harmonics
            phi(A*cos(sum(tp,2))+x1_approx)...%phi integrated over psi
            .*phip(A*cos(tp(:,1))+sum(ai,2))...%at origin we have a sum of all the cosine coeffitients
            .*ai(:,jj)...
            );
