...
%%
f1=exp(1i.*xaxis_deg-1i*pi);
r_learned=(net.phi(result.with_learning{1}.x_cl(1:100:end,:)))';
psi_hat=angle(r_learned'*f1');
ii=10;
plot(xaxis_deg,r_learned(:,ii))
hold on
plot(xaxis_deg,real(exp(1i.*xaxis_deg-1i*pi+1i*psi_hat(ii))))
%  Sort accordingly
figure
plot(xaxis_deg(1:160/10:end)-pi,psi_hat)
%% Replot
w=trained_net{1}.wfb*trained_net{1}.wout'+trained_net{1}.W;
wShift=w;
for ii=1:net.N
