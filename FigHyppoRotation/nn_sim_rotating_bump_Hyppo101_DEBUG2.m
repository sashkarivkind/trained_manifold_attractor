...
error
%% applying f_ol with input switched on
x_epsi = fast_conv_to_fp(net,...
    sim.f_ol,...
    struct('ol_with_fixed_input',1,...
    'u',[0;epsilonmag]*ones(1,size(sim.f_ol,2))));
z_epsi = net.wout'*net.phi(x_epsi);
figure;
plot(180/pi*sim.psi,angle(z_epsi(1,:)+1i*z_epsi(2,:))-sim.psi,'-')
hold on;
psi_hh = angle(sim.f_ol(1,:)+1i*sim.f_ol(2,:));
plot(180/pi*psi_hh,angle(z_epsi(1,:)+1i*z_epsi(2,:))-psi_hh,'-')
grid
%% sweeping r at the angle to which slosed loop simulation converged
x_epsi_r = fast_conv_to_fp(net,...
    sim.f_ol,...
    struct('ol_with_fixed_input',1,...
    'u',u_in(:,1000)*ones(1,size(sim.f_ol,2))));
z_epsi_r = net.wout'*net.phi(x_epsi_r);
%% running cl simulation with variety of initial conditions
x_epsi_cl = fast_conv_to_fp(net,...
    sim.f_ol,...
    struct('cl_with_fixed_input',1,...
    'u',u_in(:,1000)*ones(1,size(sim.f_ol,2))));
z_epsi_cl = net.wout'*net.phi(x_epsi_cl);
%% veryfying hat the closed loop solution holds in open loop
x_verify = fast_conv_to_fp(net,...
    z_epsi_cl,...
    struct('ol_with_fixed_input',1,...
    'u',[0;epsilonmag]*ones(1,size(sim.f_ol,2))));
z_verify = net.wout'*net.phi(x_verify);
%%
[~,ii_psi_cl]=min(abs(angle(z_verify(1,:)+1i*z_verify(2,:))-sim.psi))
delta=angle(z_epsi(1,:)+1i*z_epsi(2,:))-sim.psi;

rrr=abs(z_epsi(1,:)+1i*z_epsi(2,:));
rrr_cl=abs(z_epsi_cl(1,:)+1i*z_epsi_cl(2,:));
f_ol_tag=rrr_cl(ii_psi_cl)*[cos(sim.psi);sin(sim.psi)];

x_epsi2 = fast_conv_to_fp(net,...
    f_ol_tag,...
    struct('ol_with_fixed_input',1,...
    'u',u_in(:,1000)*ones(1,size(sim.f_ol,2))));
z_epsi2 = net.wout'*net.phi(x_epsi2);
figure;
plot(180/pi*sim.psi,angle(z_epsi2(1,:)+1i*z_epsi2(2,:))-sim.psi,'-')
grid;