...
%     pulse_width = 100./dt*[5,5,5];
%     psi_vec=[45 90 135]*pi/180;

    psi_vec=[10,30, 50 ,70 90 ]*pi/180;
    tswitch=10*[50, 150, 250, 350, 450];
    pulse_width = 100./dt*[5,5,5,5,5];
    ...
    for ttt =tswitch
        ...
        elseif  strcmp(input_mode,'hyppo')
            u_in(:,uu1:uu2)=...
                ...
                *ones(1,size(u_in(:,uu1:uu2),2));
             elseif  strcmp(input_mode,'hyppoTangent')
            u_in(:,uu1:uu2)=...
                epsilonmag*1/(a-b)*...
                [-(a-b)*sin(psi-rot)-h*(a-b)/b*cos((a-b)/b*(psi-rot));...
                (a-b)*cos(psi-rot)-h*(a-b)/b*cos((a-b)/b*(psi-rot))]./(1+h/(a-b))...
                *ones(1,size(u_in(:,uu1:uu2),2));
...
y=0.5*1/(a-b)*...
                [-(a-b)*sin(sim.psi-rot)-h*(a-b)/b*cos((a-b)/b*(sim.psi-rot));...
                (a-b)*cos(sim.psi-rot)-h*(a-b)/b*cos((a-b)/b*(sim.psi-rot))]./(1+h/(a-b));
for pp=1:size(sim.f_ol,2)
            plot([sim.f_ol(1,pp) sim.f_ol(1,pp)+y(1,pp)],[sim.f_ol(2,pp) sim.f_ol(2,pp)+y(2,pp)],'-')
end
