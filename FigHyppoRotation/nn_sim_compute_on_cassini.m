...
%             r=h*sqrt( cos(2*sim.psi-rot)+sqrt( (a/h)^4-sin(2*sim.psi-rot).^2   ) );
%             sim.f_ol=A.*[r.*cos(sim.psi-rot); r.*sin(sim.psi-rot)]; 
...
%     epsilonmag=0.01;
    epsilonmag=0.001;
    tswitch=10*[50, 200, 350];
    pulse_width = 1000*[5,5,5];
...
%             
%             r=h*sqrt( cos(2*psi-rot)+sqrt( (a/h)^4-sin(2*psi-rot).^2   ) );
%             u_in(:,uu1:uu2)=epsilonmag*[r.*cos(psi-rot); r.*sin(psi-rot)]... 
%                 *ones(1,size(u_in(:,uu1:uu2),2)); 
