...
%         hp.sim_resolution=120;
        ...
        for MM=1:length(M_vec)
            ...
            [x_cl,result.with_learning{GG,MM,TT}.z_cl]=fast_conv_to_fp_extendedTMP(net,[],...
                struct('xinit',x,...
                'ol',0,...
                'tmax',tmax,...
                'dt',dt,'save_neruons',1));
...
for GG=1:length(g_vec)
    for MM=1:length(M_vec)
        for TT=1:10
            ...
            vel2Max(GG,MM,TT)=max(mean(diff(mod(angle(result.with_learning{GG,MM,TT}.z_cl(2:end,[1 11])),2*pi),[],2),2).^2);
