...
box off;ylim([-0.03 0.03]);xlim([-pi pi])
...
figure;plot(angle(result.with_learning{1}.z_cl(2:10:end,:))','.r');
...
figure;plot(angle(result.with_learning{2}.z_cl(2:10:end,:))','.r');
box off;ylim([-pi pi]);xlim([0 1000])
if saveFlag
    if net.g==1.5
...
figure;plot(angle(result.with_learning{3}.z_cl(2:10:end,:))','.r');
box off;ylim([-pi pi]);xlim([0 1000])
